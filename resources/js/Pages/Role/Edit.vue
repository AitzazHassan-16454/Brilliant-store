...
const initializeSelectAllModules = () => {
    props.groupedPermissions.forEach((module) => {
        selectAllByModule.value[module.name] = isModuleFullySelected(module.name)
    })
    selectAllPermissions.value = areAllPermissionsSelected.value
}
    ...
    <div class="flex min-h-screen bg-[#FAF7F2] dark:bg-[#0A0A0A] transition-colors duration-300">
        ...
        <main class="flex-1 p-8">
            <div class="flex items-center justify-between mb-8 animate-fade-in-up-sm">
                <div>
                    <h2 class="text-3xl font-semibold text-[#1A1A1A] dark:text-[#F5F5F5]">Edit Role</h2>
                    <p class="text-gray-500 dark:text-[#A0A0A0] mt-2">Update role information and permissions</p>
                </div>
            </div>
